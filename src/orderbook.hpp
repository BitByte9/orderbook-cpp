...
struct Users {
    ...
    Users(const string& name)
        : user_name(name), user_balance() {}

    // Constructor with name + balance
    Users(const string& name, const Balances& balance)
        : user_name(name), user_balance(balance) {}
};
...
private:
    ...
    unordered_map <string,Users> users; // stores unique user name and balances for each user O(1) lookup no need to be in order 
    void flipBalance(const string& userId1, const string& userId2,double quantity,double price);

    public:
    OrderBook(); // constructor
   ~OrderBook(); // destructor 
    string add_bid(string Username,int Price,int Quantity); // adds a bid or ask to the order book
    string add_ask(string Username,int Price,int Quantity); // adds a bid or ask to the order book
    string getBalance(string username); // returns the balance of a user
